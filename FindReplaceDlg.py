...
class FindReplaceDlg(wx.Dialog):
    def __init__(self, parent, notebook):
        wx.Dialog.__init__(self, parent, id=wx.ID_ANY, title=u"Find - Replace",
        ...
        self.label_search_for = wx.StaticText(self, wx.ID_ANY,
                                              u"Search For:",
                                              wx.DefaultPosition,
                                              wx.DefaultSize, 0)
        left_sizer.Add(self.label_search_for, 0, wx.ALIGN_RIGHT | wx.ALL, 5)

        self.label_replace_with = wx.StaticText(self, wx.ID_ANY,
                                                u"Replace With:",
                                                wx.DefaultPosition,
                                                wx.DefaultSize, 0)
        ...
        self.text_find_str = wx.TextCtrl(self, wx.ID_ANY, wx.EmptyString,
                                         wx.DefaultPosition, wx.DefaultSize, 0)
        middle_sizer.Add(self.text_find_str, 0, wx.ALL | wx.EXPAND, 5)

        self.text_replace_str = wx.TextCtrl(self, wx.ID_ANY, wx.EmptyString,
                                            wx.DefaultPosition,
                                            wx.DefaultSize, 0)
        ...
        direction_static_sizer = wx.StaticBoxSizer(wx.StaticBox(self,
                                                                wx.ID_ANY,
                                                                u"Direction"),
                                                   wx.HORIZONTAL)

        self.radio_up = wx.RadioButton(direction_static_sizer.GetStaticBox(),
                                       wx.ID_ANY, u"Up", wx.DefaultPosition,
                                       wx.DefaultSize, wx.RB_GROUP)
        direction_static_sizer.Add(self.radio_up, 0, wx.ALL, 5)

        self.radio_down = wx.RadioButton(direction_static_sizer.GetStaticBox(),
                                         wx.ID_ANY, u"Down",
                                         wx.DefaultPosition,
                                         wx.DefaultSize, 0)
        ...
        self.button_find_next = wx.Button(self, wx.ID_ANY, u"Find Next",
                                          wx.DefaultPosition,
                                          wx.DefaultSize, 0)
        right_sizer.Add(self.button_find_next, 0, wx.ALL | wx.EXPAND, 5)

        self.button_count_all = wx.Button(self, wx.ID_ANY, u"Count All",
                                          wx.DefaultPosition,
                                          wx.DefaultSize, 0)
        right_sizer.Add(self.button_count_all, 0, wx.ALL | wx.EXPAND, 5)

        self.button_replace = wx.Button(self, wx.ID_ANY,
                                        u"Replace",
                                        wx.DefaultPosition,
                                        wx.DefaultSize, 0)
        right_sizer.Add(self.button_replace, 0, wx.ALL | wx.EXPAND, 5)

        self.button_replace_all = wx.Button(self, wx.ID_ANY,
                                            u"Replace All",
                                            wx.DefaultPosition,
                                            wx.DefaultSize, 0)
    ...
    def on_find_next(self, _):
        ...
        res, _ = te.FindText(search_start, search_end, search_str, flags)
        if res == -1:
            if self.check_wraparound.GetValue():  # wrap enabled
                if search_end != 0:  # forward wrap
                    res, _ = te.FindText(0, search_end, search_str, flags)
                    if res == -1:
                        msg = 'Nothing found!'
            ...
            if search_end == 0:  # backwards search
                te.SetCurrentPos(res)
                te.SetAnchor(res + len(search_str))
                # need to call refresh to make selection visible
                te.Refresh()
            else:
                te.SetSelection(res, res + len(search_str))
    ...
    def on_count(self, _):
        ...
        fin = True
        while fin:
            res, _ = te.FindText(search_start,
                                 search_end,
                                 search_str,
                                 flags)
            if res == -1:
        ...
        if cnt > 0:
            ...
            msg = 'Found no match!'
        wx.MessageBox(message=msg, caption='Counted!',
    ...
    def on_replace_all(self, _):
        ...
        while fin:
            ...
            if res == -1:
                ...
                search_start = res + len(search_str)
        if cnt > 0:
            msg = 'Replaced ' + str(cnt) + \
                  ' occurrence' + ('s!' if cnt > 1 else '!')
        else:
            msg = 'Found no match!'
        wx.MessageBox(message=msg, caption='Replaced!',
                      style=wx.OK | wx.ICON_INFORMATION)
